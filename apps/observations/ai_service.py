...
class ObservationAIService:
    ...
    def get_ai_feedback(observation_data=None, **kwargs):
        data = observation_data if isinstance(observation_data, dict) else {}
        # যদি আলাদাভাবে raw_notes পাঠানো হয়
        if 'raw_notes' in kwargs:
            data['raw_notes'] = kwargs['raw_notes']
...
def generate_feedback(observation_data: dict) -> dict:
    try:
        ...
        obs = ObservationData(
            teacher_name      = observation_data.get("teacher_name", "Teacher"),
            subject           = observation_data.get("subject", "N/A"),
            grade_level       = observation_data.get("grade_level", "N/A"),
        ...
        logger.error(f"❌ AI ERROR: {str(e)}")
        # ব্যাকআপ রেসপন্স (যাতে ইউজার অন্তত ডাটা দেখতে পায়)
        score = float(observation_data.get("overall_performance_score", 3.0))
        return {
            "glow": "Observation saved, but AI feedback is temporarily unavailable.",
            "grow": "Check back soon for detailed coaching feedback.",
            "overall_score": score,
            "dimensions": [],
            "error_info": str(e)
        }